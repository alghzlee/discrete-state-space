...
def do_eval(model, batchs):
    """
    Evaluasi model EWD3QN pada satu batch data.
...
def do_test(model, Xtest, actionbloctest, bloctest, Y90, SOFA, reward_value, beat):
    """
    Evaluasi model EWD3QN pada dataset uji.
    ...
    np.save('EWD3QN/shencunlv.npy', rec_sur)
    np.save('EWD3QN/agent_bQ.npy', rec_agent_q)
    np.save('EWD3QN/phys_bQ.npy', rec_phys_q)
    np.save('EWD3QN/reward.npy', rec_reward_user)
    np.save('EWD3QN/agent_actionsb.npy', rec_agent_a)
    np.save('EWD3QN/phys_actionsb.npy', rec_phys_a)
    np.save('EWD3QN/rec_agent_q_pro.npy', rec_agent_q_pro)
