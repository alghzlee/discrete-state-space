...
class dist_DQN(object):
    def __init__(self,
        ...
        self.Q = DistributionalDQN(
            state_dim, num_actions, n_atoms).to(self.device)
        self.Q_target = copy.deepcopy(self.Q)
        self.optimizer = torch.optim.Adam(
            self.Q.parameters(), lr=0.000005)  # 0.00001
        self.tau = tau
        self.gamma = gamma
        self.v_min = v_min
        self.v_max = v_max
    ...
    def compute_loss(self, batch):
        ...
        targetQ = reward + (gamma * end_multiplier *
                            (Q_dist_star+eplion*(log_Q_experience-Q_dist_star)))
